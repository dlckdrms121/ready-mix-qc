...
# 설정
# =========================
PROJECT_ROOT = r"C:\SmartConstruction_Project\SlumpGuard_Study"
...
OUT_LEFT_DIR  = os.path.join(PROJECT_ROOT, r"data\processed\roi_left")
OUT_RIGHT_DIR = os.path.join(PROJECT_ROOT, r"data\processed\roi_right")
LOG_DIR       = os.path.join(PROJECT_ROOT, r"data\processed\logs")
...
def main():
    ...
    while True:
        ...
        if left_smoothed is not None:
            ...
            if left_roi is not None:
                cv2.imwrite(os.path.join(OUT_LEFT_DIR, f"left_{frame_idx:06d}.jpg"), left_roi)

        if right_smoothed is not None:
            right_roi, right_xyxy = safe_crop(frame, right_smoothed)
            if right_roi is not None:
                cv2.imwrite(os.path.join(OUT_RIGHT_DIR, f"right_{frame_idx:06d}.jpg"), right_roi)
    ...
    out_csv = os.path.join(LOG_DIR, "b1_drop_events.csv")
